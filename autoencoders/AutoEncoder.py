...
class Autoencoder_conv2conv(object):
    def __init__(self,name,encoder_filter_size,dencoder_filter_size
                 ,input_shape,optimizer,
                 transfer_function=tf.nn.relu):
        self.weight = self._initialize_weights(name,encoder_filter_size,dencoder_filter_size)
    ...
    def _initialize_weights(self,name,encoder_filter_size,dencoder_filter_size):
        all_weights = dict()
        ...
        all_weights['w2'] = tf.get_variable(name=name + '_w2',
                                            shape=dencoder_filter_size,
                                            initializer=tf.contrib.layers.xavier_initializer(uniform=False))
        all_weights['b2'] = tf.get_variable(name=name + '_b2',
                                            shape=[dencoder_filter_size[3]],
                                            initializer=tf.contrib.layers.xavier_initializer(uniform=False))
        return all_weights
