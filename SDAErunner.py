...
ae1 = Autoencoder_conv2conv('ae1',
                              ...
                              [1,1,64,3],
                              [64,32,128,3],
                              tf.train.AdamOptimizer(0.0000085))

ae2 = Autoencoder_conv2deconv('ae2',
                              [3,3,64,96],
                              [5,5,64,96],
                              [64,32,128,64],
                              tf.train.AdamOptimizer(0.000085))
...
input = tf.placeholder(tf.float32,[64,32,128,3])
h = tf.nn.relu(tf.nn.conv2d(input,ae1.weight['w1'],[1,1,1,1],padding='SAME')+ae1.weight['b1'])#ae1
h = batch_norm(h)
...
stackcost = tf.reduce_mean(tf.square(tf.subtract(input,output)))
opt = tf.train.AdamOptimizer(0.000001).minimize(stackcost)
...
for epoch in range(2000):
    ...
    for i in range(total_batch):
        ...
        _, cost = sess.run((opt,stackcost), feed_dict={input: data.batch_size([-1, 32, 128, 3])})
        avg_cost += cost / len(data.train_data) * 64
